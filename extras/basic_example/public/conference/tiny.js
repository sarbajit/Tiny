...
{
    getRoomListWS(function(response){
       ...
       var roomList = JSON.parse(response);
        ...
        for(var i=0; i<roomList.length;i++)
        {
            ...
            option.id = roomList[i]._id;
            option.appendChild(document.createTextNode(roomList[i].name+" "+roomList[i]._id));
            roomListSelect.appendChild(option);
        }
    });
}

function getRoomListWS(callback)
{
    $.ajax({
      url:'../getRooms/',
      type: "GET",
...
{
    getUsersWS(room, function(response){
        callback(response);
...
function getUsersWS(room, callback)
{
    $.ajax({
      url:'../getUsers/'+room,
      type: "GET",
...
{
    var isEmpty = true;
    getUsers(room, function(response){
...
function createToken(room, pin, userName, role, callback) {
...
function createRoom(name, pin, callback) {
    var req = new XMLHttpRequest();
    var url = '../createRoom/';
